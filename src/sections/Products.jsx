    ...
    <section id="products" className="py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-5 lg:px-8">
        <Reveal>
          ...
          <h2 className="section-heading mt-4">Commodity Programs</h2>
          <p className="section-lead">
            Four core export categories — each backed by farmer-direct procurement, laboratory verification,
          ...
          {PRODUCTS.map((p, i) => (
            <Reveal key={p.id} delay={(i % 3) + 1}>
              <TiltCard className="glass overflow-hidden rounded-2xl">
                ...
                >
                  <img src={p.image} alt={p.name} className="h-full w-full object-cover transition duration-700 hover:scale-105" loading="lazy" />
                ...
                <div className="p-6 lg:p-8">
                  ...
                  <p
                    className={`mt-4 text-sm leading-relaxed text-cream/80 transition-all duration-500 ${
                      active === p.id ? 'max-h-40 opacity-100' : 'max-h-0 overflow-hidden opacity-0 lg:max-h-40 lg:opacity-100'
                    }`}
                  >
                    {p.detail}
                  </p>
